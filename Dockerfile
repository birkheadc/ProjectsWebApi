...
WORKDIR /src
COPY ["ProjectsWebApi.csproj", "./"]
RUN dotnet restore "ProjectsWebApi.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "ProjectsWebApi.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "ProjectsWebApi.csproj" -c Release -o /app/publish /p:UseAppHost=false
